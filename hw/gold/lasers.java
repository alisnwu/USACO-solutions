...
public class lasers{
	public static void main(String[] args) throws IOException{
		BufferedReader lin = new BufferedReader(new FileReader("lasers.in"));
		PrintWriter lout = new PrintWriter(new BufferedWriter(new FileWriter("lasers.out")));
		...
		for(int i=0; i<N; i++){
			st = new StringTokenizer(lin.readLine());
			int x = Integer.parseInt(st.nextToken());
			int y = Integer.parseInt(st.nextToken());
			if(!xtoY.containsKey(x)) xtoY.put(x, new ArrayList<Integer>());
			xtoY.get(x).add(y);
			if(!ytoX.containsKey(y)) ytoX.put(y, new ArrayList<Integer>());
			ytoX.get(y).add(x);
		}
		int ret = -1;
		while(!q.isEmpty()){
